...
vector<double> powerMethod(vector<vector<double>> A, vector<double> initialX){
    ...
    while(k <= N){
        ...
        vector<double> absoluteErr = vectorPlusMinus(x, numVecMultiplier(1/y[maxPos], y), -1);
        double ERR = absoluteErr[vectorNormInfPos(absoluteErr)];
        x = numVecMultiplier(1/y[maxPos], y);
        // cout << ERR << endl;
        if(abs(ERR) < TOL){
...
int main(){
    vector<vector<double>> A = {{-2, -3}, {6, 7}};
    vector<double> x = {1, 1};
    vector<double> eigen = powerMethod(A, x);
    return 0;
}